...
<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc8-33"
      data-genuitec-path="/red_wine/WebRoot/sys/profile/profile.jsp">
<!-- 页面标题 -->
<div id="TitleArea" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc8-33"
     data-genuitec-path="/red_wine/WebRoot/sys/profile/profile.jsp">
...
<div id="MainArea">
    ...
    <div class="profile_box">
        <c:choose>
            <c:when test="${not empty result}">
                <textarea name="content" class="TextareaStyle"
                          style="width: 80%;height: 300px;padding: 10px;border: 1px solid #91C0E3;">${result.content}</textarea>
                <input style="display:none" name="id" value="${result.id}"/>
            </c:when>
            <c:otherwise>
                <textarea name="content" style="width: 80%;height: 300px;padding: 10px;border: 1px solid #91C0E3;"
                          class="TextareaStyle"> 暂无数据！</textarea>
            </c:otherwise>
        </c:choose>
    </div>
</div>
